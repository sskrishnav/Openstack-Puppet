...
def create_instance(request, instance_params):
    ...
    key_name  = instance_params['keyname']
...
# NOTE(Harikrishna): Cinder API Calls goes here

def volume_create(request, params):
    """ Method that handels the volume creation."""
    name = params['vname']
    size = params['vsize']
    desc = params['vdes']
    vtype = params['vtype']
    ...
    try:
        volume_info = api.cinder.volume_create(request, size, name, desc, vtype)
    except Exception as e:
        logger.error('volume_create: Unable to create Volume')
        raise e
    return volume_info

def volume_list(request):
    """ Methood that retrieves the volume list."""
    return api.cinder.volume_list(request)

def volume_get(request, volume_id):
    """ Method that gets the volume information."""
    result = api.cinder.volume_get(request, volume_id)
    return result.status


def volume_attach(request, instance_id, volume_id, device=None):
    """ Metho that attaches a volume to the server."""
    return api.nova.instance_volume_attach(request, volume_id, instance_id, device)
...
def volume_detach(request):
    ...
    try:
        ...
        attachment_id = volume.attachments[0]['attachment_id']
        status = api.nova.instance_volume_detach(request, instance_id, volume_id)
    except IndexError:
        pass
    except Exception as e:
        logger.error(e)
...
def volume_delete(request):
    volume_id = request.GET.get('volume_id')
    try:
       status = api.cinder.volume_delete(request, volume_id)
    except Exception as e:
        logger.error('Unable to delete the volume')
    return

# NOTE(Harikrishna): Nova API Call's related to keypair

def keypair_create(request, keypair_name):
    return api.nova.keypair_create(request, keypair_name)

def keypair_import(request, keypair_name, public_key):
    return api.nova.keypair_import(request, keypair_name, public_key)

def keypair_list(request):
    return api.nova.keypair_list(request)

def keypair_delete(request, keypair_id):
    return api.nova.keypair_delete(request, keypair_id)
