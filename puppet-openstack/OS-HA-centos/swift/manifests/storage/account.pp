...
class swift::storage::account(
  ...
  Swift_config<| |> ~> Service['openstack-swift-account-reaper']
  Swift_config<| |> ~> Service['openstack-swift-account-auditor']
  ...
  service { 'openstack-swift-account-reaper':
    ...
    #require  => Package['opensatck-swift-account'],
  }

  service { 'openstack-swift-account-auditor':
    ensure   => $service_ensure,
