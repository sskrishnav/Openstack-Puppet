...
class glance::api(
  ...
  $auth_type                = 'keystone',
  $auth_host                = undef,
  #$auth_url                 = "http://hiera(castlight::controller::address::management):35357/v2.0",
  $auth_url                 = undef,
  $auth_port                = '35357',
  $auth_uri                 = false,
  ...
  if $::osfamily == 'Debian'{
     ...
     File {
      ...
      Glance_api_config<||>   ~> Exec<| title == 'glance-manage db_sync' |>
      Glance_cache_config<||> ~> Exec<| title == 'glance-manage db_sync' |>
      Exec<| title == 'glance-manage db_sync' |> ~> Service['openstack-glance-api']
      Glance_api_config<||>   ~> Service['openstack-glance-api']
      Glance_cache_config<||> ~> Service['openstack-glance-api']
      Class['glance::policy'] ~> Service['openstack-glance-api']
      Service['openstack-glance-api']   ~> Glance_image<||>
  ...
  if $sql_connection {
    ...
    #$database_connection_glance = "$database_connection$glance_username:$keystone_password@$host/$glance_username"
  ...
  if $::osfamily == 'Debian' {
     ...
     service { 'openstack-glance-api':
       ...
       hasstatus  => true,
       hasrestart => true,
       #require    => File['/var/log/glance/api.log']
     }
  }
  ...
  if $::osfamily == 'RedHat' {
     ...
     $validation_options_hash = merge ($defaults, $validation_options)
     create_resources('openstacklib::service_validation', $validation_options_hash, {'subscribe' => 'Service[openstack-glance-api]'})
     }
  }
