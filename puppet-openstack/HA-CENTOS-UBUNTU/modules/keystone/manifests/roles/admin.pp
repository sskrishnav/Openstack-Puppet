...
class keystone::roles::admin(
  ...
  $service_tenant_desc    = 'Tenant_for_the_openstack_services',
  $configure_user         = true,
  $configure_user_role    = true,
) {
  notify{"in keystone::endpoint file ----------->":}
  notify{"$admin ----->  $admin_tenant  --------> $service_tenant":}
  ...
  if $configure_user {
    ...
    #Keystone_tenant[$admin_tenant] -> Keystone_user[$admin]
    #Keystone_tenant[$service_tenant] -> Keystone_user[$admin]
  }
  ...
  if $configure_user_role {
    ...
    #Keystone_user[$admin] -> Keystone_role['admin'] -> Keystone_user_role["${admin}@${admin_tenant}"]
  }
  #Keystone_user_role["${admin}@${admin_tenant}"] -> Keystone_service <||>
