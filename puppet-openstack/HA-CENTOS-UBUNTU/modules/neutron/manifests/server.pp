...
class neutron::server (
  ...
  if $sync_db {
    if ($::neutron::params::server_package) {
      ...
      if $::osfamily == 'RedHat' {
         Package<| title == 'openstack-neutron' |> ~> Exec['neutron-db-sync']
      }
    }
  ...
  if ($::neutron::params::server_package) {
    ...
    # The neutron api config file is provided by the neutron package.
        if $::osfamily == 'RedHat' {
           Package['openstack-neutron'] -> Class['neutron::policy']
           Package['openstack-neutron'] -> Neutron_api_config<||>
        }
  ...
  if ($auth_type == 'keystone') {
    ...
    #  }
      ...
      #  'filter:authtoken/username':          value => $auth_user;
      #  'filter:authtoken/password':          value => $auth_password, secret => true;
      #}
...
#        }
        ...
        #TODO(Harikrishna): auth_host, auth_port and auth_protocol is no longer used, have to remove
#        if $auth_host {
...
#      }
      ...
      if $auth_url {
        ...
        $auth_url_real = "${auth_protocol}://${auth_host}:35357/"
      }
