...
class heat(
  ...
  package { "$::heat::params::common_package_name":
    ensure => $package_ensure,
    name   => $::heat::params::common_package_name,
  }
  Package["$::heat::params::common_package_name"] -> Heat_config<||>
  ...
  if $rpc_backend == 'heat.openstack.common.rpc.impl_kombu' {
    ...
    if $rabbit_hosts {
      ...
      heat_config { 'oslo_messaging_rabbit/rabbit_userid': value => $rabbit_userid }
      heat_config { 'oslo_messaging_rabbit/rabbit_password': value => $rabbit_password }
  ...
  if $auth_uri {
    ...
    heat_config { 'keystone_authtoken/identity_uri': value => "${keystone_protocol}://${keystone_host}:35357"; }
  } else {
