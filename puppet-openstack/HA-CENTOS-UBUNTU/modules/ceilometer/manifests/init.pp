...
class ceilometer(
  ...
  File {
      require => Package["$::ceilometer::params::common_package_name"],
  }
  ...
  package { "$::ceilometer::params::common_package_name":
    ...
    Package["$::ceilometer::params::common_package_name"] -> Ceilometer_config<||>
  ...
  if $rpc_backend == 'ceilometer.openstack.common.rpc.impl_kombu' {
    ...
    if $rabbit_hosts {
      ...
      ceilometer_config { 'DEFAULT/auth_strategy': value => 'keystone' }
  ...
  ceilometer_config {
    ...
    'publisher/telemetry_secret'      : value => $metering_secret, secret => true;
