...
class ceilometer::collector (
  ...
  if $::osfamily == 'Debian' {
    ...
    Ceilometer_config<||> ~> Service['openstack-ceilometer-collector']

    Package[$::ceilometer::params::collector_package_name] -> Service['openstack-ceilometer-collector']
    ensure_resource( 'package', [$::ceilometer::params::collector_package_name],
      { ensure => $package_ensure }
    )
    ...
    if $manage_service {
      if $enabled {
        $service_ensure = 'running'
        Class['ceilometer::db'] -> Service['openstack-ceilometer-collector']
        Exec['ceilometer-dbsync'] ~> Service['openstack-ceilometer-collector']
      } else {
        $service_ensure = 'stopped'
      }
    }

    Package['openstack-ceilometer-common'] -> Service['openstack-ceilometer-collector']
